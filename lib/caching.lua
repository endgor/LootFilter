...
function LootFilter.processCaching()
	...
	if (slots < LootFilterVars[LootFilter.REALMPLAYER].freebagslots) and (table.getn(LootFilter.cleanList) > 0) then
		...
		if (LootFilter.deleteItemFromBag(LootFilter.cleanList[1])) then
			...
			LootFilter.print(LootFilter.cleanList[1]["link"] ..
			" " ..
			LootFilter.Locale.LocText["LTWasDeleted"] ..
			": " .. LootFilter.Locale.LocText["LTItemLowestValue"] .. " (" .. calculatedValue .. ")");
	...
	if (table.getn(LootFilter.cleanList) >= 1) then -- if we have one or more items on the list start selling/deleting
		...
		if (LootFilterButtonDeleteItems:GetText() == LootFilter.Locale.LocText["LTSellItems"]) then -- sell items
			while LootFilter.sellQueue < LootFilter.SELL_QUEUE do
				...
				LootFilter.schedule(LootFilter.SELL_INTERVAL, LootFilter.deleteItems, GetTime() + LootFilter
				.SELL_TIMEOUT, false);
			end;


			-- Safety check: Ensure item is not in keepList (double check)
			local keepReason = LootFilter.matchKeepNames(item);
			if (keepReason == "") then
				keepReason = LootFilter.matchKeepProperties(item);
			end;

			if (keepReason ~= "") then
				LootFilter.print("Skipped selling kept item: " .. item["link"] .. " (" .. keepReason .. ")");
			else
			...
			LootFilter.schedule(LootFilter.SELL_INTERVAL, LootFilter.checkIfItemSold,
				GetTime() + LootFilter.SELL_ITEM_TIMEOUT, item);
